...
Dans cette partie nous travaillerons sur la mise à jour de la quantité du produit pour nous assurer que chaque commande livrera le produit réel. Actuellement, le modèle de produit n’a pas d’attribut de quantité. Alors ajoutons le:
...
Et n'oublions pas de mettre à jour les _fixtures_ en ajoutant le champs `quantity`.
...
Et n'oublions pas de mettre à jour l'action `create` du `ProductController` pour prendre en compte ce nouvel attribut lors de la création du produit:
...
C'est pour cela que je pense que les https://github.com/typeorm/typeorm/blob/master/docs/listeners-and-subscribers.md[`Subscribers` de TypeORM] sont un bien meilleur endroit pour gérer ça. Cela va nous permettre de déplacer toute cette logique dans une nouvelle classe.
