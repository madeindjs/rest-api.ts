...
// TODO add invrisy exppress utils later ?.
...
NOTE: Les contrôleurs font partis du _design patern_ *MVC: Modèle, Vue, Contrôleur*. Leur but est d'intercepter la requête et d'appeler les services dédiés.
...
La classe en elle même est vraiment très simple. La chose a noter ici est la présence d'un décorateur `injectable`. Ce décorateur va simplement https://github.com/inversify/InversifyJS/blob/master/src/annotation/injectable.ts#L12[ajouter une metadata] a notre classe afin qu'elle puisse être injecté dans nos futures dépendances.
...
Passons maintenant à la suite pour voir à quoi va nous servir ce `container`.

==== Le contrôleur

Le contrôleur utilisera le décorateur `@controller`. Ce décorateur va lui aussi déclarer ce contrôleur comme `@injectable` mais aussi nos offrir des fonctionnalités spéciales.
...
Et il faut maintenant importer ce contrôleur dans le container que nous avons définis précédemment. C'est vraiment très simple à faire :
...
Maintenant que tout est prêt, il ne nous reste plus qu'à modifier notre fichier pour démarrer l'application. Pour ceci il suffit de https://github.com/inversify/inversify-express-utils[suivre la documentation]. `inversify-express-utils` nous facilite vraiment la tâche :
