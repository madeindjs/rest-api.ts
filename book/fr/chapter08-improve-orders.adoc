...
Et ajoutons aussi un test qui va vérifier que nous ne pouvons créer un produit avec un stock négatif:
...
Faire passer le test est très facile gràce aux décorateurs de TypeORM. Il suffit d'ajouter le décorateur `@IsInt` et `@Min` comme cela:
...
Et voilà. Commitons les changements:
