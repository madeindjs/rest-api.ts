...
export class UsersController {
  ...
  @httpPost("/")
  public async create(@requestBody() body: Partial<User>, req: Request, res: Response) {
    const repository = await this.databaseService.getRepository(UserRepository);
    const user = new User();
  ...
  public async destroy(@requestParam("userId") userId: string, req: Request & { user: User }, res: Response) {
    if (Number(userId) !== req.user.id) {
      return res.sendStatus(403);
    }
    const repository = await this.databaseService.getRepository(UserRepository);
