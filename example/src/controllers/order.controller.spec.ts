...
import { generateOrder, generateProduct, generateUser } from "../tests/faker.utils";
...
describe("OrderController", () => {
  ...
  describe.skip("update", () => {
    it("should update order", (done) => {
      ...
      agent.put(`/orders/${order.id}`).set("Authorization", jwt).send({ title, price, published }).expect(204, done);
    });

    it("should not update product of other users", (done) => {
      const { price, published } = generateProduct();
      agent.put(`/orders/${order.id}`).set("Authorization", strangerJwt).send({ price, published }).expect(403, done);
    });

    it("should not update product without auth", (done) => {
      const { price, published } = generateProduct();
      agent.put(`/orders/${order.id}`).send({ price, published }).expect(403, done);
    });
  });
  ...
  describe.skip("destroy", () => {
    it("should destroy order", (done) => {
      ...
      agent.delete(`/orders/${order.id}`).set("Authorization", jwt).expect(204, done);
    });
    ...
    it("should not destroy of other users", (done) => {
      agent.delete(`/orders/${order.id}`).set("Authorization", strangerJwt).expect(403, done);
    });
  });
});
