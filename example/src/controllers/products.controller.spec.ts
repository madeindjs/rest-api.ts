...
describe("ProductsController", () => {
  ...
  describe("create", () => {
    it("should create product", (done) => {
      ...
      agent.post("/products").set("Authorization", jwt).send({ title, price, published }).expect(201, done);
    });
    ...
    it("should not create user with missing title", (done) => {
      ...
      agent.post("/products").set("Authorization", strangerJwt).send({ price, published }).expect(400, done);
    });
  });
  ...
  describe("update", () => {
    ...
    it("should not update product without auth", (done) => {
      const { price, published } = generateProduct();
      agent.put(`/products/${product.id}`).send({ price, published }).expect(403, done);
    });
  });
  ...
  describe("destroy", () => {
    it("should destroy product", (done) => {
      ...
      agent.delete(`/products/${product.id}`).set("Authorization", jwt).expect(204, done);
    });
    ...
    it("should not destroy of other users", (done) => {
      agent.delete(`/products/${product.id}`).set("Authorization", strangerJwt).expect(403, done);
    });
  });
});
